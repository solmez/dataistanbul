mydata <- read.csv("height-wind.csv", head=T, sep=",")
height <- mydata$Height
windsp <- mydata$Wind.speed
fit0 <- lm(height ~ windsp)
plot(fit0$fitted.values,fit0$residuals,pch=16)
qqnorm(fit0$residuals,pch=16)
qqline(fit0$residuals)
print(summary(fit0))
plot(windsp,height, xlab="Wind Speed", ylab="Height", col="red")
n <- length(height)
Nreps <- 1000

...
for (iter in 1:Nreps) {
  ...
  ybs <- append(ybs,fitbs$coefficients[[1]])  #intercept
  xbs <- append(xbs,fitbs$coefficients[[2]])  #slope 
  abline(fitbs, col="gray")
}
abline(fit0, col="blue")
#
cat("Bootstrapped intercept & std. error: ", mean(ybs), sd(ybs), '\n')
cat("CI for the intercept               : ", quantile(ybs, c(0.025,0.975)), '\n')
cat("Bootstrapped slope & std. error    : ", mean(xbs), sd(xbs), '\n')

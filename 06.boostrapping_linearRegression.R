...
#plot the 95% confidence interval for the fit
newx <- seq(min(windsp), max(windsp), 0.1)
conf <- predict(fit0, newdata=data.frame(windsp=newx), interval=c("confidence"), level = 0.95, type="response")
prd <- predict(fit0, newdata=data.frame(windsp=newx), interval=c("prediction"), level = 0.95, type="response")
